...
export const AddConnectionDialog = ({
  ...
  const handleProviderSelect = (providerId: string, isCustomForm: boolean = false) => {
    if (isCustomForm) {
      ...
      <DialogContent showOverlay={true}>
        <DialogHeader>
          <DialogTitle>
            {selectedProvider === 'imap'
              ...
              : t('pages.settings.connections.connectEmail')}
          </DialogTitle>
          <DialogDescription>
            {selectedProvider === 'imap'
              ? 'Enter your IMAP email credentials to connect your account'
              : t('pages.settings.connections.connectEmailDescription')}
          </DialogDescription>
        </DialogHeader>
        {!canCreateConnection && !selectedProvider && (
        ...
        {selectedProvider === 'imap' ? (
          <div className="mt-4">
            ...
            <ImapForm onClose={() => onOpenChange?.(false)} />
          </div>
        ) : (
          <motion.div
              ...
              return (
                ...
                >
                  <Button
                    ...
                    onClick={
                      provider.isCustomForm
                        ? () => handleProviderSelect(provider.providerId, true)
                        : async () =>
